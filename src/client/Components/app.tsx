...
export default class App extends React.Component<AppProps, AppStates> {
  ...
  testPost = async (): Promise<void> => {
    ...
    if (textOfPostTest.trim()) {
      try {
        const res: ITest = await Post(apiRoute.getRoute('test'), {
          text: textOfPostTest
        })
        this.setState({
  ...
  testPut = async (): Promise<void> => {
    ...
    if (response && textOfPutTest.trim()) {
      try {
        const res: ITest = await Put(apiRoute.getRoute('test'), {
          text: textOfPutTest,
          id: response?._id
        })
      ...
      this.setState({
        textForPut:
          "You don't have any resource in database to change. first use post"
      })
    }
  }
  ...
  testDelete = async (): Promise<void> => {
    ...
    if (response) {
      try {
        const res: ITest = await Delete(apiRoute.getRoute('test'), {
          id: response?._id
        })
        this.setState({
          textForDelete: `${res._id} ${res.text}`,
          response: undefined
        })
      } catch (e) {
      ...
      this.setState({
        textForDelete:
          "You don't have any resource in database to delete. first use post"
      })
    }
  }
